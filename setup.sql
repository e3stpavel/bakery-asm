...
(
    ...
    CONSTRAINT fk_users_user_roles_join_users FOREIGN KEY (user_id) REFERENCES users (user_id) ON UPDATE CASCADE,
...
(
    ...
    CONSTRAINT chk_sessions_session_code CHECK ( LENGTH(TRIM(session_code)) > 0 AND LENGTH(TRIM(session_code)) <= 254 ),
    CONSTRAINT chk_sessions_expires_at CHECK ( expires_at BETWEEN strftime('%s', '1970-01-01') AND strftime('%s', '2100-12-31') ),
    CONSTRAINT fk_sessions_users FOREIGN KEY (user_id) REFERENCES users (user_id) ON UPDATE CASCADE
);
...
(
    ...
    CONSTRAINT chk_assets_asset_code CHECK ( LENGTH(TRIM(asset_code)) > 0 AND LENGTH(TRIM(asset_code)) <= 254 ),
    ...
    CONSTRAINT chk_assets_deleted_at_deleted_by_id CHECK ( (deleted_at IS NULL AND deleted_by_id IS NULL) OR
                                                           (deleted_at IS NOT NULL AND deleted_by_id IS NOT NULL AND
                                                            status_code = 'STORE') ),
    ...
    CONSTRAINT fk_assets_users_created_by FOREIGN KEY (created_by_id) REFERENCES users (user_id) ON UPDATE CASCADE,
    CONSTRAINT fk_assets_users_updated_by FOREIGN KEY (updated_by_id) REFERENCES users (user_id) ON UPDATE CASCADE,
    CONSTRAINT fk_assets_users_deleted_by FOREIGN KEY (deleted_by_id) REFERENCES users (user_id) ON UPDATE CASCADE
);
...
INSERT
INTO Assets (asset_code, name, description, status_code, ownership_code, condition_code, acquired_at, acquisition_price,
             expected_lifespan, created_by_id, updated_by_id)
VALUES ('s6gysid1xwjhcful23bus04d', 'Industrial Oven', 'Large capacity oven for baking bread', (SELECT id FROM inUse),
        (SELECT id FROM owned), (SELECT id FROM operational), strftime('%s', 'now'), 1000, 8, (SELECT id FROM user),
        (SELECT id FROM user)),
       ('owp63iikb5xabpbh7p3hls91', 'Dough Mixer', 'Heavy-duty mixer for dough preparation', (SELECT id FROM inStorage),
       ...
       ('cmecowy42vhqi5ipcmms9lr0', 'Delivery Van', 'Refrigerated van for transporting goods',
        (SELECT id FROM underMaintenance), (SELECT id FROM leased), (SELECT id FROM damaged), strftime('%s', 'now'),
